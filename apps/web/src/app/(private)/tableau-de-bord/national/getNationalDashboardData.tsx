...
const aggregateConumCras = (
  ...
  conumCras.reduce<CraConseillerNumeriqueParDepartement>(
    (accumulator, current) => {
      for (const key of Object.keys(accumulator)) {
        const keyTyped = key as keyof CraConseillerNumeriqueParDepartement
        const value = accumulator[keyTyped]
        const currentValue = current[keyTyped]

        if (typeof value !== 'number' || typeof currentValue !== 'number') {
          return accumulator
        }
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        accumulator[keyTyped] = (value + currentValue) as never
      }
