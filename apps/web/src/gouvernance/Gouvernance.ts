...
export type CodePorteurGouvernance = 'region' | 'departement' | 'epci'
export const getPorteurCode = (type: CodePorteurGouvernance, code: string) =>
  `${type}#${code}`

export const getInfoFromPorteurCode = (porteurCode: string) => {
  const [type, code] = porteurCode.split('#')
  return { type, code } as {
    type: CodePorteurGouvernance
    code: string
  }
}

export const perimetreOptions: Option<PerimetreGouvernance>[] = [
  {
    value: 'epci',
...
export const GouvernanceValidation = z.intersection(
  z
