...
export const createJobExecutionCron = (
  ...
  const command = `/usr/bin/curl -v --fail --max-time 21600 https://${apiHostname}/api/jobs --header Content-Type: application/json --header x-api-key: ${internalApiPrivateKey} --data ${escapedJsonData}`
